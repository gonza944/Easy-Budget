...
const { members } = storeToRefs(useMyMembersStore());
...
const filteredParticipants = computed(() => {
    ...
    let filtered = members.value.filter(participant =>
        ...
        <FormField v-slot="{ componentField }" name="participants">
            <FormItem v-auto-animate>
                ...
                <FormControl>
                    ...
                    -->
                    <Combobox v-model:open="open" :model-value="componentField.modelValue || []" :ignore-filter="true"
                        ...
                        <ComboboxAnchor as-child>
                            ...
                            -->
                            <TagsInput :model-value="componentField.modelValue || []" class="px-2 gap-2 w-full"
                                :display-value="(value) => typeof value === 'string' ? value : (value as Member).display_name"
                                @update:model-value="componentField.onChange">
                                <div class="flex gap-2 flex-wrap items-center">
                                    <!-- Each selected participant as a removable tag -->
                                    <TagsInputItem v-for="participant in (componentField.modelValue || [])"
                                        :key="participant.id" :value="participant" class="px-3 py-4">
                        ...
                        <ComboboxList class="w-[--reka-popper-anchor-width]">
                            ...
                            <ComboboxGroup>
                                ...
                                -->
                                <ComboboxItem v-for="participant in filteredParticipants" :key="participant.id"
                                    :value="participant" class="px-4" @select.prevent="(event) => {
                                        // Cast the selected value to Member type for type safety
                                        const selectedParticipant = event.detail.value as Member;
